...
pub fn ProjectSelectForm(
    ...
    let (general_error_message, 
        ...
        dest_error_message, 
        validation_check) = validation_derived_signals(
                                            source_project_rw, dest_project_rw, projects_rw, projects_validated_rw
                                        );
    ...
    view!(
        ...
        <select
            ...
            prop:value=move || source_project_rw.get().to_string()>
                <Suspense fallback=move || view! { <option value="">Loading projects...</option> }>
                    {move || {
                        if !projects_rw.get().is_empty() && source_project_rw.get().is_empty() {
                            source_project_rw.set(projects_rw.get()[0].id.clone());
                        }
        ...
        <Suspense fallback=move || view! { <option value="">Loading projects...</option> }>
            {move || {
                if !projects_rw.get().is_empty() && dest_project_rw.get().is_empty() {
                    dest_project_rw.set(projects_rw.get()[0].id.clone());
                }
...
fn validation_derived_signals(    
    source_project_rw: RwSignal<String>,
    dest_project_rw: RwSignal<String>,
    projects_rw: RwSignal<Vec<Project>>,
    projects_validated_rw: RwSignal<bool>,
) -> (Memo<String>, Memo<String>, Memo<String>, Memo<String>){
    ...
    (source_error_message, dest_error_message, general_error_message, validation_check)
}